...
export default function PilihSendiri3({ navigation, route }) {
  ...
  const totalTransaksi = produk.reduce((accumulator, object) => {
    return accumulator + parseFloat(object.qty * object.harga_produk);
  }, 0);

  const totalQty = produk.reduce((accumulator, object) => {
  ...
  const handlePesan = () => {
    if (
      ...
      axios.post(apiURL + 'transaksi_add', {
        fid_user: data.id,
        lokasi: lokasi.length == 0 ? data.alamat : lokasi,
        tanggal_ambil: selectedDate,
        jenis: siapPakaiChecked ? 'Siap Pakai' : 'Cuci Saja',
